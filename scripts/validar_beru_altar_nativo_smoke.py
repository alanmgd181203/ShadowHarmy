...
async def _probar() -> None:
    ...
    assert motivo == "replantada"
    assert movida is not None and movida.exito
    assert bridge.canceladas == 1 and bridge.creadas == 2
    assert long.altar_revision == 1
    ...
    assert creada_mariscal.exito and bridge.creadas == 3
    ...
    assert bridge.canceladas == 2 and bridge.creadas == 4
    assert mariscal.altar_revision == 1
