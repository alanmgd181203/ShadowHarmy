#!/usr/bin/env python3
"""Flota Beru viva 100% — Hoz real en toda la legión.

Ojos reales · ambas campanas · margen spot ON · sin techo de ensayo.
Cada Santo nace en el grado que sostiene su manto (Soldado→Mariscal).
Igris y Greed hibernan. Bitácora única papel/vivo.
...
os.environ["ARISE_BERU_FLOTA_VIVA"] = "true"
os.environ["MODO_SIMULACION"] = "false"
os.environ["BERU_MANOS"] = "true"
os.environ["BERU_MANOS_FANTASMA"] = "true"
os.environ["BERU_MANOS_ACTIVOS"] = ",".join(FLOTA)
os.environ["BERU_MANOS_EXIGIR_TIER"] = "AUTO"
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Beru flota viva — Hoz real en los 22 Santos, grado = manto",
    )
    ap.add_argument(
        "--segundos",
        type=float,
    ...
    print(
        f"\n[FLOTA VIVA] {len(FLOTA)} Santos · Hoz real · grado = manto · "
        f"{seg:.0f}s · Igris OFF\n",
        flush=True,
    )
